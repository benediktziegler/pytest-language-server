...
// ============================================================================
// Rename Fixture Tests
// ============================================================================

#[test]
fn test_rename_fixture_at_definition() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Get rename locations from the definition (line 5 in conftest, 0-indexed = line 4)
    // "def my_fixture" - my_fixture starts at char 4
    let result = db.collect_rename_locations(&conftest_path, 4, 5);
    assert!(result.is_ok(), "Should find rename locations");

    let info = result.unwrap();
    assert_eq!(info.definition.name, "my_fixture");
    assert_eq!(info.locations.len(), 2, "Should have definition + 1 usage");

    // Check that both locations are present
    let has_definition = info
        .locations
        .iter()
        .any(|loc| loc.file_path == conftest_path && loc.line == 5);
    assert!(has_definition, "Definition location should be included");

    let has_usage = info
        .locations
        .iter()
        .any(|loc| loc.file_path == test_path && loc.line == 2);
    assert!(has_usage, "Usage location should be included");
}

#[test]
fn test_rename_fixture_at_usage() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Get rename locations from the usage in test file (line 2, 0-indexed = line 1)
    // "def test_something(my_fixture)" - my_fixture starts around char 19
    let result = db.collect_rename_locations(&test_path, 1, 20);
    assert!(result.is_ok(), "Should find rename locations from usage");
...
#[test]
fn test_rename_fixture_multiple_usages_same_file() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let info = result.unwrap();
    // 1 definition + 3 usages
    assert_eq!(
        info.locations.len(),
        4,
        "Should have 1 definition + 3 usages"
    );
}

#[test]
fn test_rename_fixture_multiple_files() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let test1_path = PathBuf::from("/tmp/test/test_one.py");
    db.analyze_file(test1_path.clone(), test1_content);
...
"#;
    ...
    let test2_path = PathBuf::from("/tmp/test/test_two.py");
    db.analyze_file(test2_path.clone(), test2_content);
    ...
    let info = result.unwrap();
    // 1 definition + 2 usages (from 2 different files)
    assert_eq!(
        info.locations.len(),
        3,
        "Should have 1 definition + 2 usages"
    );

    // Verify usages are from different files
    let file_paths: std::collections::HashSet<_> = info
        .locations
        .iter()
    ...
    assert_eq!(
        file_paths.len(),
        3,
        "Should have locations in 3 different files"
    );
}

#[test]
fn test_rename_conftest_fixture_used_in_subdir() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let root_conftest_path = PathBuf::from("/tmp/project/conftest.py");
    db.analyze_file(root_conftest_path.clone(), root_conftest_content);

    let subdir_test_content = r#"
def test_uses_root(root_fixture):
    pass
"#;

    let subdir_test_path = PathBuf::from("/tmp/project/subdir/test_sub.py");
    db.analyze_file(subdir_test_path.clone(), subdir_test_content);

    let result = db.collect_rename_locations(&root_conftest_path, 4, 5);
    assert!(result.is_ok());
    ...
    assert_eq!(
        info.locations.len(),
        2,
        "Should have definition + usage in subdir"
    );

    let has_subdir_usage = info
        .locations
        .iter()
        .any(|loc| loc.file_path == subdir_test_path);
    assert!(has_subdir_usage, "Should include usage in subdirectory");
}

#[test]
fn test_rename_subdir_conftest_fixture_not_affects_root() {
    let db = FixtureDatabase::new();

    // Root conftest with a fixture
    let root_conftest_content = r#"
import pytest
...
"#;
    ...
    // Subdir conftest with SAME fixture name (override)
    let subdir_conftest_content = r#"
import pytest
...
"#;
    ...
    let subdir_conftest_path = PathBuf::from("/tmp/project/subdir/conftest.py");
    db.analyze_file(subdir_conftest_path.clone(), subdir_conftest_content);

    // Test in subdir uses the subdir fixture
    let subdir_test_content = r#"
def test_uses_local(local_fixture):
    pass
"#;
    ...
    // Test in root uses the root fixture
    let root_test_content = r#"
def test_uses_root(local_fixture):
    pass
"#;

    let root_test_path = PathBuf::from("/tmp/project/test_root.py");
    db.analyze_file(root_test_path.clone(), root_test_content);

    // Rename from subdir conftest - should only affect subdir
    let result = db.collect_rename_locations(&subdir_conftest_path, 4, 5);
    assert!(result.is_ok());

    let info = result.unwrap();
    // Should only have subdir definition + subdir test usage
    assert_eq!(
        info.locations.len(),
        2,
        "Should have subdir definition + subdir usage"
    );

    // Should NOT include root conftest or root test
    let has_root_conftest = info
        .locations
        .iter()
        .any(|loc| loc.file_path == root_conftest_path);
    assert!(!has_root_conftest, "Should NOT include root conftest");

    let has_root_test = info
        .locations
        .iter()
        .any(|loc| loc.file_path == root_test_path);
    assert!(!has_root_test, "Should NOT include root test");
}

#[test]
fn test_rename_overriding_fixture_only_renames_child() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let parent_conftest_path = PathBuf::from("/tmp/project/conftest.py");
    db.analyze_file(parent_conftest_path.clone(), parent_conftest_content);

    // Child conftest overrides with same name
    let child_conftest_content = r#"
import pytest

@pytest.fixture
def cli_runner(cli_runner):
    return cli_runner + "_child"
"#;

    let child_conftest_path = PathBuf::from("/tmp/project/tests/conftest.py");
    db.analyze_file(child_conftest_path.clone(), child_conftest_content);

    let test_content = r#"
def test_uses_child(cli_runner):
    pass
"#;
    ...
    // Rename from child definition (line 5, the function name "cli_runner")
    let result = db.collect_rename_locations(&child_conftest_path, 4, 5);
    assert!(result.is_ok());

    let info = result.unwrap();
    // Should have: child definition + test usage
    // The parameter "cli_runner" in child conftest refers to PARENT, so it should NOT be renamed
    assert_eq!(info.definition.file_path, child_conftest_path);

    // Should NOT include parent conftest definition
    let has_parent = info
        .locations
        .iter()
        .any(|loc| loc.file_path == parent_conftest_path);
    assert!(!has_parent, "Should NOT rename parent fixture");
}

#[test]
fn test_rename_self_referencing_fixture_at_param() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Child conftest overrides
    let child_conftest_content = r#"
import pytest

@pytest.fixture
def base_fixture(base_fixture):
...
"#;
    ...
    // Rename from the PARAMETER position (refers to parent)
    // Line 5: "def base_fixture(base_fixture):" - the param starts around char 18
    let result = db.collect_rename_locations(&child_conftest_path, 4, 20);
    assert!(result.is_ok());

    let info = result.unwrap();
    // This should resolve to the PARENT fixture
    assert_eq!(
        info.definition.file_path, parent_conftest_path,
        "Should resolve to parent"
    );
}

#[test]
fn test_rename_fixture_in_usefixtures_decorator() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let info = result.unwrap();
    // Should have definition + usefixtures usage
    assert_eq!(
        info.locations.len(),
        2,
        "Should include usefixtures decorator usage"
    );

    let has_decorator_usage = info.locations.iter().any(|loc| {
        loc.file_path == test_path && loc.line == 4 // @pytest.mark.usefixtures line
    });
    assert!(has_decorator_usage, "Should include usefixtures decorator");
}

#[test]
fn test_rename_fixture_in_parametrize_indirect() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let info = result.unwrap();
    // Definition + parametrize string + function param
    assert!(
        info.locations.len() >= 2,
        "Should include parametrize indirect usage"
    );
}

#[test]
fn test_rename_fixture_does_not_affect_different_scope() {
    let db = FixtureDatabase::new();

    // Fixture in subdir A
    let subdir_a_conftest_content = r#"
import pytest
...
"#;
    ...
    let subdir_a_conftest_path = PathBuf::from("/tmp/project/subdir_a/conftest.py");
    db.analyze_file(subdir_a_conftest_path.clone(), subdir_a_conftest_content);

    let subdir_a_test_content = r#"
def test_a(local_fixture):
    pass
"#;

    let subdir_a_test_path = PathBuf::from("/tmp/project/subdir_a/test_a.py");
    db.analyze_file(subdir_a_test_path.clone(), subdir_a_test_content);

    // Same fixture name in subdir B (independent)
    let subdir_b_conftest_content = r#"
import pytest
...
"#;
    ...
    let subdir_b_conftest_path = PathBuf::from("/tmp/project/subdir_b/conftest.py");
    db.analyze_file(subdir_b_conftest_path.clone(), subdir_b_conftest_content);

    let subdir_b_test_content = r#"
def test_b(local_fixture):
    pass
"#;

    let subdir_b_test_path = PathBuf::from("/tmp/project/subdir_b/test_b.py");
    db.analyze_file(subdir_b_test_path.clone(), subdir_b_test_content);

    // Rename from subdir A - should NOT affect subdir B
    let result = db.collect_rename_locations(&subdir_a_conftest_path, 4, 5);
    assert!(result.is_ok());

    let info = result.unwrap();
    // Should only have A's definition + A's test usage
    assert_eq!(info.locations.len(), 2);

    let has_b_conftest = info
        .locations
        .iter()
        .any(|loc| loc.file_path == subdir_b_conftest_path);
    assert!(!has_b_conftest, "Should NOT include subdir B conftest");

    let has_b_test = info
        .locations
        .iter()
        .any(|loc| loc.file_path == subdir_b_test_path);
    assert!(!has_b_test, "Should NOT include subdir B test");
}

#[test]
fn test_rename_fixture_with_dependency_chain() {
    let db = FixtureDatabase::new();
...
@pytest.fixture
def top_fixture(derived_fixture):
    return derived_fixture + "_top"
"#;
    ...
    let info = result.unwrap();
    // base_fixture definition + derived_fixture's parameter usage
    assert_eq!(info.locations.len(), 2);

    let has_derived_usage = info.locations.iter().any(|loc| {
        loc.file_path == conftest_path && loc.line == 9 // derived_fixture line
    });
    assert!(has_derived_usage, "Should include usage in derived_fixture");
}

#[test]
fn test_rename_cannot_rename_builtin_fixtures() {
    use pytest_language_server::RenameError;

    let db = FixtureDatabase::new();

    let test_content = r#"
def test_something(request, tmp_path, monkeypatch):
    pass
"#;
    ...
    // Try to rename "request" - should fail
    let result = db.collect_rename_locations(&test_path, 1, 20);
    assert!(matches!(result, Err(RenameError::CannotRenameBuiltin(_))));
}

#[test]
fn test_rename_cannot_rename_third_party_fixtures() {
    use pytest_language_server::RenameError;

    let db = FixtureDatabase::new();

    // Simulate a third-party fixture in site-packages
    let mocker_content = r#"
import pytest
...
"#;
    ...
    let mocker_path = PathBuf::from("/tmp/venv/lib/python3.9/site-packages/pytest_mock/plugin.py");
    db.analyze_file(mocker_path.clone(), mocker_content);
...
"#;
    ...
    // Try to rename from the usage - should resolve to third-party and fail
    let result = db.collect_rename_locations(&test_path, 1, 20);
    assert!(matches!(
        result,
        Err(RenameError::CannotRenameThirdParty(_))
    ));
}

#[test]
fn test_rename_valid_python_identifier() {
    assert!(FixtureDatabase::is_valid_python_identifier("my_fixture"));
    assert!(FixtureDatabase::is_valid_python_identifier("_private"));
    assert!(FixtureDatabase::is_valid_python_identifier("MyFixture123"));
    assert!(FixtureDatabase::is_valid_python_identifier("fixture_v2"));
}

#[test]
fn test_rename_invalid_python_identifier() {
    assert!(!FixtureDatabase::is_valid_python_identifier(""));
    assert!(!FixtureDatabase::is_valid_python_identifier("123start"));
    assert!(!FixtureDatabase::is_valid_python_identifier("has-dash"));
    assert!(!FixtureDatabase::is_valid_python_identifier("has space"));
    assert!(!FixtureDatabase::is_valid_python_identifier("def")); // keyword
    assert!(!FixtureDatabase::is_valid_python_identifier("class"));
    assert!(!FixtureDatabase::is_valid_python_identifier("return"));
}

#[test]
fn test_rename_no_fixture_at_position() {
    use pytest_language_server::RenameError;
...
"#;
    ...
    // Try to rename at a position with no fixture
    let result = db.collect_rename_locations(&test_path, 2, 5);
    assert!(matches!(result, Err(RenameError::NoFixtureAtPosition)));
}

#[test]
fn test_get_fixture_range_at_position_usage() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Get range at usage position
    let result = db.get_fixture_range_at_position(&test_path, 1, 20);
    assert!(result.is_some());

    let (name, _line, start, end) = result.unwrap();
    assert_eq!(name, "my_fixture");
    assert!(start < end);
}

#[test]
fn test_get_fixture_range_at_position_definition() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Get range at definition position (line 5, "def my_fixture")
...
#[test]
fn test_rename_fixture_char_positions_correct() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let test_content = r#"
def test_something(my_fixture, other_param):
    pass
"#;
    ...
    // Check that character positions are accurate
    for loc in &info.locations {
        // Verify end > start
        assert!(
            loc.end_char > loc.start_char,
            "end_char should be > start_char"
        );
        // Verify the span matches the fixture name length
        let span = loc.end_char - loc.start_char;
        assert_eq!(
            span,
            "my_fixture".len(),
            "Span should match fixture name length"
        );
    }
}

#[test]
fn test_rename_fixture_in_class() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let test_path = PathBuf::from("/tmp/test/test_class.py");
    db.analyze_file(test_path.clone(), test_content);

    let result = db.collect_rename_locations(&test_path, 4, 5);
    assert!(result.is_ok());

    let info = result.unwrap();
    // Definition + usage in class method
    assert_eq!(info.locations.len(), 2);

    let has_class_usage = info.locations.iter().any(|loc| loc.line == 9);
    assert!(has_class_usage, "Should include usage in class method");
}

#[test]
fn test_rename_async_fixture() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    let result = db.collect_rename_locations(&conftest_path, 4, 11); // "async def async_fixture"
    assert!(result.is_ok());
...
#[test]
fn test_rename_fixture_deep_hierarchy() {
    let db = FixtureDatabase::new();
...
"#;
    ...
    // Level 1 conftest
    let l1_conftest_content = r#"
import pytest

@pytest.fixture
def l1_fixture(root_fixture):
    return root_fixture + "_l1"
"#;
    let l1_conftest_path = PathBuf::from("/tmp/project/level1/conftest.py");
    db.analyze_file(l1_conftest_path.clone(), l1_conftest_content);

    // Level 2 test
    let l2_test_content = r#"
def test_deep(root_fixture, l1_fixture):
    pass
"#;
    let l2_test_path = PathBuf::from("/tmp/project/level1/level2/test_deep.py");
    db.analyze_file(l2_test_path.clone(), l2_test_content);

    // Rename root_fixture - should include usages at all levels
    let result = db.collect_rename_locations(&root_conftest_path, 4, 5);
    assert!(result.is_ok());

    let info = result.unwrap();
    // root_fixture definition + l1_fixture param + l2 test param
    assert_eq!(
        info.locations.len(),
        3,
        "Should have 3 locations across hierarchy"
    );
}

#[test]
fn test_is_builtin_fixture() {
    assert!(FixtureDatabase::is_builtin_fixture("request"));
    assert!(FixtureDatabase::is_builtin_fixture("tmp_path"));
    assert!(FixtureDatabase::is_builtin_fixture("monkeypatch"));
    assert!(FixtureDatabase::is_builtin_fixture("capsys"));
