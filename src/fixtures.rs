...
impl FixtureDatabase {
    ...
    pub fn analyze_file(&self, file_path: PathBuf, content: &str) {
        ...
        let is_conftest = file_path.file_name().map(|n| n == "conftest.py").unwrap_or(false);
    ...
    fn visit_stmt(&self, stmt: &Stmt, file_path: &PathBuf, _is_conftest: bool, content: &str) {
        ...
        debug!("Function {} has {} decorators", func_name, decorator_list.len());
        ...
        if is_fixture {
            ...
            info!("Found fixture definition: {} at {:?}:{}", func_name, file_path, line);
            ...
            for arg in &args.args {
                ...
                if arg_name != "self" && arg_name != "request" {
                    info!("Found fixture dependency: {} at {:?}:{}", arg_name, file_path, func_line);
        ...
        if func_name.starts_with("test_") {
            ...
            for arg in &args.args {
                let arg_name = arg.def.arg.as_str();
                if arg_name != "self" {
                    info!("Found fixture usage: {} at {:?}:{}", arg_name, file_path, func_line);
    ...
    fn visit_assignment_fixture(&self, assign: &rustpython_parser::ast::StmtAssign, file_path: &PathBuf, content: &str) {
                ...
                if self.is_fixture_decorator(&inner_call.func) {
                    ...
                    for target in &assign.targets {
                        if let Expr::Name(name) = target {
                            ...
                            info!("Found fixture assignment: {} at {:?}:{}", fixture_name, file_path, line);
    ...
    fn is_fixture_decorator(&self, expr: &Expr) -> bool {
        match expr {
            Expr::Name(name) => {
    ...
    pub fn find_fixture_definition(
        ...
        debug!("find_fixture_definition: file={:?}, line={}", file_path, line);
        ...
        let fixture_name = usages.iter().find(|usage| {
            debug!("Comparing usage.line={} with target_line={}", usage.line, target_line);
    ...
    fn find_closest_definition(&self, file_path: &Path, fixture_name: &str) -> Option<FixtureDefinition> {
        ...
        debug!("Checking for fixture {} in same file: {:?}", fixture_name, file_path);
        for def in definitions.iter() {
            if def.file_path == file_path {
                info!("Found fixture {} in same file (highest priority)", fixture_name);
        ...
        debug!("Searching for fixture {} in conftest.py files starting from {:?}", fixture_name, current_dir);
        loop {
            ...
            for def in definitions.iter() {
                if def.file_path == conftest_path {
                    info!("Found fixture {} in conftest.py: {:?}", fixture_name, conftest_path);
                    return Some(def.clone());
                }
            }
        ...
        warn!("No fixture {} found following priority rules, returning first available", fixture_name);
    ...
    pub fn find_fixture_at_position(
        ...
        debug!("find_fixture_at_position: file={:?}, line={}, char={}", file_path, target_line, character);
        ...
        for entry in self.definitions.iter() {
            for def in entry.value().iter() {
                if def.file_path == file_path && def.line == target_line {
                    ...
                    if let Some(ref word) = word_at_cursor {
                        if word == &def.name {
                            info!("Found fixture definition name at cursor position: {}", def.name);
        ...
        for entry in self.usages.iter() {
            ...
            for usage in usages.iter() {
                if usage.name == fixture_name {
                    debug!("Found reference to {} in {:?} at line {}", fixture_name, file_path, usage.line);
                    all_references.push(usage.clone());
                }
            }
        }
        
        info!("Found {} total references for fixture: {}", all_references.len(), fixture_name);
        all_references
    }
}
...
"#;
        ...
        assert_eq!(fixture_usages.len(), 2, "Should detect fixture usage in both async and sync tests");
    }
}
